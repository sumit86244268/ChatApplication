...
function SearchDiv({ user, results, userDetails, inviteDiv }) {
    ...
    const fetchUsersInvites = async () => {
        try {
            const res = await axios.get(`https://chatapplication-f34s.onrender.com/api/invites/received/${userDetails.username}`);
    ...
    const sendInvite = async (senderUsername, receiverUsername) => {
        const res = await axios.post('https://chatapplication-f34s.onrender.com/api/invites/send' + `?senderUsername=` + senderUsername + `&receiverUsername=` + receiverUsername);
        console.log(res.data);
    };
    ...
    const statusSet = async (inviteId, receiverDt) => {
        try {
            const res = await axios.post(
                `https://chatapplication-f34s.onrender.com/api/invites/respond`,
                null,
